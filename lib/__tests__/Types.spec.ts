...
declare function GivenType<T>(callback: (x: T) => void): void;

{
  ...
  // TODO: Expose GivenType in Typebolt library

  GivenType<Tuple<number>>(tuple =>
    tuple.map(x => {
      Assert<IsExactType<number, typeof x>>();
    })
  );
}
